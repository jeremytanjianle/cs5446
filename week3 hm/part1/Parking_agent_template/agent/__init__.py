...
class GeneratePDDL_Stationary :
    ...
    def generateInitString(self) :
        ...
        init_string += f"(at agent1 pt{agent_gym_ojb.position.x}pt{agent_gym_ojb.position.y}) "

        # add cars
        # e.g. '(at car4 pt4pt2) (at car2 pt6pt1)'
        car_set = self.state.cars
        # for car in car_set:
        #     init_string += f"(at car{car.id} pt{car.position.x}pt{car.position.y}) "
        for car in car_set:
            init_string += f"(blocked pt{car.position.x}pt{car.position.y}) "
    ...
    def generateGoalString(self) :
        ...
        return f"(at agent1 pt{self.state.finish_position.x}pt{self.state.finish_position.y})" 
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="up", 
                ...
                #   precondition_string="(and (at ?agent ?pt1) (not (blocked ?pt2)) )", 
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")
                #   precondition_string="(and (at ?pt1 ?agent) (not (blocked ?pt2)) )", 
    ...
    gen.addAction(name="forward", 
                  parameters=(("agent", "agent"), ("pt1" , "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="( and (at ?agent ?pt1) (not (blocked ?pt2)) (forward_next ?pt1 ?pt2) )", 
                #   precondition_string="( and (at ?agent ?pt1) (not (blocked ?pt2))  )", 
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")
                #   precondition_string="(and (at ?pt1 ?agent) (not (blocked ?pt2)) )", 
                #   effect_string= "(and (not (at ?pt1 ?agent)) (at ?pt2 ?agent))")
