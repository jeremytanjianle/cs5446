...
class GeneratePDDL_Stationary :
    ...
    def generateGridCells(self) :
        ...
        for w in range(self.width) :
            for lane in range(self.num_lanes) :
                for t in range(self.max_time): # 3rd dimension of time in grids
    ...
    def generateInitString(self) :
        ...
        # add agent
        agent_gym_ojb = self.state.agent
        init_string += f"(at agent1 pt{agent_gym_ojb.position.x}pt{agent_gym_ojb.position.y}pt0) "

        # add cars
        # e.g. '(at car4 pt4pt2pt0) (at car2 pt6pt1pt0)'
        car_set = self.state.cars
        for car in car_set:
            speed_range = car.speed_range
            assert speed_range[0] == speed_range[1], f"{car.id} car range {speed_range} has more than 1 unique elements"
            car_speed = speed_range[0]*-1
            init_string += f"(blocked pt{car.position.x}pt{car.position.y}pt{0}) "
            for t in range(1, self.max_time):
                for trail in range(1, car_speed+1):
                    # if car speed = -3, trail is {1,2,3}
                    new_x = car.position.x - (t-1)*car_speed - trail
                    while new_x < 0:
                        new_x += self.width
                    # if car.id == 18: print(car.position.x, t*car_speed, trail, new_x, t)
                    init_string += f"(blocked pt{new_x}pt{car.position.y}pt{t}) "
    ...
    def generateGoalString(self) :
        ...
        goal_string = "(or "
        for t in range(1, self.max_time):
            goal_string += f"(at agent1 pt{self.state.finish_position.x}pt{self.state.finish_position.y}pt{t})"
        goal_string += ")"
        return goal_string
    ...
    def generateProblemPDDL(self) :
        ...
        # print(PDDL_FILE_ABSOLUTE_PATH + self.problem_file_name)
...
def generateDomainPDDLFile(gen):
    ...
    gen.addAction(name="up", 
                  ...
                  precondition_string="(and (at ?agent ?pt1) (not (blocked ?pt2)) (up_next ?pt1 ?pt2) )",
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")

    gen.addAction(name="down", 
                  parameters=(("agent", "agent"), ("pt1" , "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="(and (at ?agent ?pt1) (not (blocked ?pt2)) (down_next ?pt1 ?pt2) )", 
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")

    gen.addAction(name="forward[-1]", 
                  parameters=(("agent", "agent"), ("pt1" , "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="( and (at ?agent ?pt1) (not (blocked ?pt2)) (forward_next_1 ?pt1 ?pt2) )", 
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")

    gen.addAction(name="forward[-2]", 
                  parameters=(("agent", "agent"), ("pt1" , "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="( and (at ?agent ?pt1) (not (blocked ?pt2)) (forward_next_2 ?pt1 ?pt2) )", 
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")

    gen.addAction(name="forward[-3]", 
                  parameters=(("agent", "agent"), ("pt1" , "gridcell"), ("pt2", "gridcell")), 
                  precondition_string="( and (at ?agent ?pt1) (not (blocked ?pt2)) (forward_next_3 ?pt1 ?pt2) )", 
                  effect_string= "(and (not (at ?agent ?pt1)) (at ?agent ?pt2))")
...
def runPDDLSolver(gen):
    ...
    '''
    command =    FAST_DOWNWARD_DIRECTORY_ABSOLUTE_PATH + 'fast-downward.py ' + PDDL_FILE_ABSOLUTE_PATH + gen.domain_file_name + ' ' + PDDL_FILE_ABSOLUTE_PATH + gen.problem_file_name + ' --search  \"lazy_greedy([ff()], preferred=[ff()])\"' + ' > temp '
...
def test() :
    ...
    '''
    env=gym.make('GridDriving-v0', lanes=LANES, width=WIDTH, random_seed=RANDOM_SEED, agent_speed_range=AGENT_SPEED_RANGE)
    # print(env.actions)
