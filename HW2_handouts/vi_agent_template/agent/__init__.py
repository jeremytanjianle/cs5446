...
def value_iteration(trn_fn, rwd_fn, gamma):
    ...
    '''
    value_fn_prime = value_fn.copy()
    # while True:
    # print(trn_fn[:,1])
    while True:
        # set value function
        value_fn = value_fn_prime.copy()

        for state in range(rwd_fn.shape[0]): # 9*5*2 = 90

            # action = int(policy[state])
            # print(f"action: {action}") # 0

            # prob_next_state = trn_fn[action][state]
            prob_next_state_for_any_action = trn_fn[:,state]
            # print(prob_next_state_for_any_action.sum()) # 5
            
            expected_reward_of_next_state = matmul(prob_next_state_for_any_action, value_fn_prime ) # shape: (5,)
            expected_reward_of_next_state += matmul(prob_next_state_for_any_action, rwd_fn[state] ) # actual reward
            # print(f"expected_reward_of_next_state: {expected_reward_of_next_state.shape}") # 0.0

            best_action, best_reward = np.argmax(expected_reward_of_next_state), np.max(expected_reward_of_next_state)
            # print(f"best_action, best_reward : {best_action, best_reward}")

            # debug
            # if state==1:
            #     print("state 1")
            #     print(immediate_reward)
            #     print(gamma*best_reward)
            #     print(expected_reward_of_next_state)

            value_fn_prime[state] = gamma*best_reward
            policy[state] = int(best_action)

        change_in_v_function = (value_fn_prime - value_fn).max()
        # print(value_fn_prime)
        
        if change_in_v_function < delta: 
            break
    # print(f"policy: {policy}")
    # print(f"value_fn_prime: {value_fn_prime}")
...
def extractValueAndPolicy(env, lanes, width, gamma):
    ...
    tran = mdp.get_transition_function() # (9, 5, 2, 5, 9, 5, 2)
    rwd = mdp.get_reward_function() # (9, 5, 2, 9, 5, 2)
